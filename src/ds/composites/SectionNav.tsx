  ...
  return (
    ...
    >
      {items.map((item, index) => {
        ...
        const classes = cn(
          'inline-flex h-9 items-center rounded-t-default border border-b-0 px-3.5 text-ui font-medium whitespace-nowrap transition-colors focus-visible:outline-hidden focus-visible:shadow-ring-inset',
          isActive
        ...
        const countBadge = badge !== undefined && badge !== null ? (
          <span
            className={cn(
              'ml-1.5 inline-flex h-5 min-w-5 items-center justify-center rounded-pill border px-1.5 text-meta leading-none',
              isActive
                ? 'border-on-dark-border bg-on-dark-active text-on-dark'
