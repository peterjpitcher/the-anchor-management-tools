...
interface ReceiptTableRowProps {
  ...
  onRemove: (id: string, previousStatus: ReceiptTransaction['status']) => void
    ...
    <tr className="align-top hover:bg-gray-50/50">
      ...
      <td className="px-4 py-3">
         <div className="flex flex-col gap-2">
             ...
             {transaction.status !== 'no_receipt_required' && <Button variant="secondary" size="xs" onClick={() => handleStatusUpdate('no_receipt_required')} disabled={isPending || !canManageReceipts}>Skip</Button>}
             {transaction.status !== 'cant_find' && <Button variant="secondary" size="xs" onClick={() => handleStatusUpdate('cant_find')} className="border border-rose-200 text-rose-700" disabled={isPending || !canManageReceipts}>Missing</Button>}
