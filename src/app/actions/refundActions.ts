...
/**
 * A manager overriding the seasonal refund cutoff. The reason is required, because "the rule said
 * no and a named manager overrode it, for this reason" is the thing that has to be on the record.
 */
export type RefundPolicyOverride = {
  overrideRefundPolicy?: boolean
  overrideReason?: string
}

/**
 * Enforces the refund promise a guest was actually shown when they paid a seasonal deposit.
 *
 * Until now that promise was generated by `describeRefundPolicy`, served to the website, printed
 * on the booking, and enforced by absolutely nothing: a refund inside the window went through
 * exactly like one outside it. The terms are read from the BOOKING's snapshot, never from
 * `booking_periods`, so editing or archiving a period cannot retrospectively change what someone
 * was promised.
 *
 * Bookings with no seasonal snapshot are ordinary large-group deposits and are left completely
 * alone: they never carried this promise and their behaviour is unchanged.
 */
async function enforceSeasonalRefundPolicy(
  db: ReturnType<typeof createAdminClient>,
  sourceType: SourceType,
  sourceId: string,
  userId: string,
  reason: string,
  options?: RefundPolicyOverride,
  ...
  const decision = resolveSeasonalDepositRefund({
    bookingDate: bookingTerms.booking_date,
    refundCutoffDays: Number(cutoff),
    // Entitlement is all we need here; the manager has already typed the amount.
    depositAmount: 0,
  })

  if (decision.entitled) return {}

  if (options?.overrideRefundPolicy !== true) {
    return {
      error:
        `${decision.reason} ` +
        'To refund it anyway, use the manager override and give a reason.',
    }
  }

  const overrideReason = (options.overrideReason || '').trim()
  if (!overrideReason) {
    return { error: 'A manager override needs a reason, which is recorded against the refund.' }
  }
  ...
  await logAuditEvent({
    ...
    additional_info: {
      action: 'seasonal_refund_policy_override',
      booking_period_code: bookingTerms.booking_period_code ?? null,
      booking_period_name: bookingTerms.booking_period_name ?? null,
      refund_cutoff_days: decision.cutoffDays,
      days_before_booking: decision.daysBefore,
      policy_shown_to_guest: bookingTerms.deposit_refund_policy ?? null,
...
async function sendPrivateBookingRefundEmailSideEffect(
  ...
  try {
    const { data: pb } = await db
      ...
      .eq('id', sourceId)
      .maybeSingle()
    if (!pb?.contact_email || pb.status !== 'completed') return false
...
export async function processPayPalRefund(
  ...
  if (policyCheck.error) return { error: policyCheck.error }
  ...
  try {
    ...
    if (result.status === 'COMPLETED') {
      ...
      // Awaited, not fired and forgotten, because its answer decides whether the generic
      // refund email goes out at all. A completed private booking used to get both: the
      // itemised deposit email AND the generic one, which disagreed with it.
