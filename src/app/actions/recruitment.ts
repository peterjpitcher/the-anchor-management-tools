...
async function requireRecruitmentUser() {
  ...
  const { data, error } = await admin.rpc('get_user_roles', { p_user_id: user.id })
  if (error) {
    throw error
  }

  const hasSuperAdmin = (data ?? []).some((role: any) => role.role_name === 'super_admin')
  if (!hasSuperAdmin) {
    throw new Error('Recruitment management is restricted to super admins.')
  }
...
export async function issueRecruitmentBookingInviteAction(formData: FormData): Promise<ActionResult> {
  try {
    ...
    const booking = await issueRecruitmentBookingLink(applicationId, type)
...
export async function inviteRecruitmentCandidateAsEmployeeAction(formData: FormData): Promise<ActionResult> {
  try {
    ...
    await completeRecruitmentHireHandoff(applicationId, invite.employeeId, admin)
    revalidatePath('/recruitment')
