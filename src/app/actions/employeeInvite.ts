...
export async function inviteEmployee(prevState: any, formData: FormData) {
  ...
  try {
    ...
    try {
      await sendWelcomeEmail(email, buildOnboardingUrl(row.token));
    } catch (emailError) {
    ...
    return { type: 'success', message: `Invite sent to ${email}.`, employeeId: row.employee_id };
  } catch (err: any) {
