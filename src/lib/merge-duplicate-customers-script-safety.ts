...
export function isMergeDuplicateCustomersMutationEnabled(
  ...
  return (
    ...
    isTruthyEnv(env.RUN_MERGE_DUPLICATE_CUSTOMERS_MUTATION)
  )
}
...
export function assertMergeDuplicateCustomersMutationAllowed(
  ...
  assertScriptMutationAllowed({
    ...
    envVar: 'ALLOW_MERGE_DUPLICATE_CUSTOMERS_MUTATION_SCRIPT'
  })
}
...
export function readMergeDuplicateCustomersLimit(
  ...
  return (
    ...
    parseOptionalPositiveInt(env.MERGE_DUPLICATE_CUSTOMERS_LIMIT)
  )
}
...
export function readMergeDuplicateCustomersOffset(
  ...
  return (
    ...
    parseOptionalNonNegativeInt(env.MERGE_DUPLICATE_CUSTOMERS_OFFSET)
  )
}

export function assertMergeDuplicateCustomersLimit(limit: number | null, hardCap: number): number {
  if (limit === null) {
    throw new Error('merge-duplicate-customers blocked: --limit is required in mutation mode.')
  }

  if (!Number.isFinite(limit) || limit <= 0) {
    throw new Error('merge-duplicate-customers blocked: --limit must be a positive integer.')
  }

  if (limit > hardCap) {
    throw new Error(
      `merge-duplicate-customers blocked: --limit ${limit} exceeds hard cap ${hardCap}. Run in smaller batches.`
    )
  }
