...
export async function extractRecruitmentCandidateFromCv(
  ...
  const response = await callOpenAIJson({
    ...
    system: [
      ...
      'Concerns must be about role fit, experience gaps, availability, travel, missing evidence, or operational risks only.',
...
export async function draftRecruitmentEmail(
  ...
  const response = await callOpenAIJson({
    ...
    system: [
      ...
      'Use only supplied context. Rejections must be generic and must not include internal concerns.',
    ].join(' '),
    user: `Email context:\n${JSON.stringify(requestInput, null, 2)}`,
    schema: {
      type: 'object',
