 ...
 *
 * A path that sets `delayedFallbackAllowed` must add its renderer here in the same change. The
 * table booking renderer covers every message sent through notifyTableBookingGuestEmailFirst.
 */
export const DELAYED_FALLBACK_RENDERERS: DelayedFallbackRenderer[] = [
  privateBookingFallbackRenderer,
  tableBookingFallbackRenderer,
]
