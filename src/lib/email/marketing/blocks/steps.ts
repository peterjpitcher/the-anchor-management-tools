...
export const steps = defineBlock<StepsData>({
  ...
  render: (data) => {
    const rows: string[] = [
      ...
      `<tr><td bgcolor="${GUEST.cream}" class="gutter" style="background-color:${GUEST.cream};padding:32px 32px 8px;font-family:'DM Serif Display',Georgia,'Times New Roman',serif;font-size:26px;line-height:32px;letter-spacing:-0.02em;color:${GUEST.green}">${escapeEmailText(data.heading)}</td></tr>`,
    ...
    data.steps.forEach((step, index) => {
      rows.push(
        `<tr><td width="44" valign="top" style="width:44px;padding:18px 0 0"><table role="presentation" cellpadding="0" cellspacing="0" border="0" style="border-collapse:separate"><tbody><tr><td align="center" bgcolor="${GUEST.green}" width="32" height="32" style="width:32px;height:32px;background-color:${GUEST.green};border-radius:999px;font-family:'Outfit',Helvetica,Arial,sans-serif;font-size:15px;font-weight:600;color:${GUEST.surface}">${index + 1}</td></tr></tbody></table></td><td valign="top" style="padding:18px 0 0"><div style="font-family:'Outfit','Helvetica Neue',Helvetica,Arial,sans-serif;font-size:16px;font-weight:600;line-height:24px;color:${GUEST.text}">${escapeEmailText(step.title)}</div><div style="font-family:'Outfit','Helvetica Neue',Helvetica,Arial,sans-serif;font-size:14px;line-height:22px;color:${GUEST.textMuted}">${escapeEmailText(step.body)}</div></td></tr>`,
      )
    })
