...
export const mediaRow = defineBlock<MediaRowData>({
  ...
  render: (data) => {
    ...
    return [
      ...
      `<tr>`,
      `<td width="240" valign="top" class="stack" style="width:240px;padding:0"><a href="${href}" style="text-decoration:none"><img src="${src}" width="${data.image.width}" height="${data.image.height}" alt="${escapeEmailText(data.image.alt)}" style="display:block;width:100%;max-width:100%;height:auto;border:1px solid ${GUEST.border}"></a></td>`,
      `<td width="24" class="stack" style="width:24px;font-size:0;line-height:0;height:16px">&nbsp;</td>`,
      `<td valign="middle" class="stack" style="padding:0"><div style="font-family:${SERIF};font-size:22px;line-height:28px;letter-spacing:-0.02em;color:${GUEST.green};padding-bottom:8px">${escapeEmailText(data.heading)}</div><div style="font-family:${SANS};font-size:14px;line-height:23px;color:${GUEST.text};padding-bottom:12px">${escapeEmailText(data.body)}</div><div style="font-family:${SANS};font-size:14px;font-weight:600;line-height:20px"><a href="${href}" style="color:${GUEST.accentText};text-decoration:none">${escapeEmailText(data.link_label)}</a></div></td>`,
      `</tr>`,
      `</tbody></table></td></tr>`,
      `</tbody></table>`,
    ].join('\n')
