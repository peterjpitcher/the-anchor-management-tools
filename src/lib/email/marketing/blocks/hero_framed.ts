...
<tr><td bgcolor="${GUEST.greenDeep}" style="background-color:${GUEST.greenDeep};padding:26px"><table role="presentation" width="548" cellpadding="0" cellspacing="0" border="0" style="width:548px;border-collapse:collapse;border:1px solid rgba(201,160,32,0.55)"><tbody>
<tr><td align="center" style="padding:44px 34px 40px">
<div style="font-family:'Outfit','Helvetica Neue',Helvetica,Arial,sans-serif;font-size:11px;font-weight:600;line-height:16px;letter-spacing:0.18em;text-transform:uppercase;color:${GUEST.goldBright};padding-bottom:14px">${escapeEmailText(data.kicker)}</div>
<div style="font-family:'DM Serif Display',Georgia,'Times New Roman',serif;font-size:36px;line-height:42px;letter-spacing:-0.02em;color:${GUEST.creamText};padding-bottom:14px">${escapeEmailText(data.headline)}</div>
<div style="font-family:'Outfit','Helvetica Neue',Helvetica,Arial,sans-serif;font-size:16px;line-height:26px;color:${GUEST.creamText};padding-bottom:24px">${escapeEmailText(data.body)}</div>
