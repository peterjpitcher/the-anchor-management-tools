...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className = '', variant = 'primary', size = 'md', children, asChild = false, ...props }, ref) => {
    ...
    const baseStyles = 'inline-flex items-center justify-center rounded-lg font-medium transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed whitespace-nowrap min-h-[44px]';
    ...
    switch (size) {
      case 'sm':
        ...
        break;
      case 'md':
        sizeStyles = 'px-6 py-3 md:py-2 text-base md:text-sm';
        break;
      case 'lg':
