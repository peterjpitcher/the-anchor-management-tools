...
import { CategoryCustomerSuggestions } from '@/components/features/customers/CategoryCustomerSuggestions'
...
export function AddAttendeesModalWithCategories({
  ...
  const excludedCustomerIds = useMemo(() => {
    ...
    // Add all customers with bookings (including reminder-only ones)
    currentBookings.forEach((b: BookingLike) => {
      if (b.customer_id) excluded.add(b.customer_id)
    })
    // Add all checked-in customers
    checkIns.forEach(checkIn => {
      if (checkIn.customer?.id) excluded.add(checkIn.customer.id)
    })
    return excluded
  }, [currentBookings, checkIns])

  const availableCustomers = useMemo(() => {
    const filtered = allCustomers
      .filter(customer => !excludedCustomerIds.has(customer.id))
      .filter(customer => {
  ...
  }, [allCustomers, excludedCustomerIds, searchTerm])
  ...
  const handleCategorySuggestionsSelect = (customerIds: string[], candidateIds: string[]) => {
    ...
    const allowedCandidates = customerIds.filter(id => !excludedCustomerIds.has(id))
