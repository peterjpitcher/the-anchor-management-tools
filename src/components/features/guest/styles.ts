...
export const GUEST_KICKER_CLASS =
  'font-anchor-body text-xs font-semibold uppercase leading-none tracking-[0.18em] text-guest-accent-text'
...
export const GUEST_LEAD_CLASS =
  'font-anchor-body text-guest-lead leading-[1.6] text-guest-text-muted'
 ...
 *
 * `text-base` is deliberate and must not be reduced: anything smaller makes
...
export const GUEST_INPUT_CLASS =
  'block min-h-[48px] w-full rounded-guest-field border-[1.5px] border-guest-border-strong bg-guest-surface px-4 py-3 font-anchor-body text-base leading-[1.4] text-guest-text transition-[border-color,box-shadow] duration-200 placeholder:text-guest-text-muted focus:border-anchor-gold-dark focus:shadow-guest-focus'
...
export const GUEST_SUNK_BOX_CLASS =
  'rounded-guest-card border border-guest-border bg-guest-sunk p-4 font-anchor-body text-ui leading-[1.6] text-guest-text'

/** A label row wrapping a tick box or radio. Never let this drop below 44px. */
export const GUEST_CHOICE_ROW_CLASS =
  'flex min-h-[44px] cursor-pointer items-center gap-[11px] font-anchor-body text-sm leading-[1.5] text-guest-text'
