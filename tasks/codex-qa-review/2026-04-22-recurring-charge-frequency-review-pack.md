# Review Pack: recurring-charge-frequency
...
Check billing cron logic, period determination, quarterly/annual edge cases, backwards compatibility, and database constraint correctness.

## Spec

Source: `tasks/recurring-charge-frequency-spec.md`
...
Invoice line items for recurring charges already use `description_snapshot` and `amount_ex_vat_snapshot` from instances. The period covered will be visible in the invoice notes via the `period_start`/`period_end` fields. No changes needed to the invoice line item builder.

## Edge Cases

1. **Changing frequency on an existing charge**: If a charge is changed from monthly to quarterly mid-quarter, the next billing run should check if an instance already exists for the relevant period. The unique constraint prevents duplicates.

2. **Disabling a charge mid-period**: A disabled charge won't generate new instances (the cron already filters by `is_active`). Any existing unbilled instances for past periods are still eligible for billing.

3. **First-run for a new quarterly/annual charge**: If a quarterly charge is created in February, it won't bill until April (when Q1 ends). This is correct behaviour — it will first appear on the Q1 bill.

4. **Carried-forward instances**: The cron already handles unbilled instances from previous periods. A quarterly instance from Q4 2025 that was never billed will be picked up in the next run regardless of current period.
