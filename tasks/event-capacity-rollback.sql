-- Restores the live definitions captured before this migration. No booking rows changed.
BEGIN;
...
END;
$preserve$;
DROP TRIGGER guard_event_physical_capacity ON public.events;
DROP FUNCTION public.guard_event_physical_capacity_v01();
...
DROP FUNCTION public.event_seating_inventory_v01(timestamptz,timestamptz,boolean);
-- Stop if somebody has subsequently changed a frozen allowance.
DO $rollback$
...
('e9e84ee8-c59b-4f93-80f6-7e7961a03240'::uuid, 11)) AS frozen(id, standing) LOOP
    SELECT standing_capacity INTO v FROM public.events WHERE id = r.id FOR UPDATE;
    IF v IS DISTINCT FROM r.standing THEN
      RAISE EXCEPTION 'event_capacity_rollback_changed: %', r.id;
    END IF;
    UPDATE public.events SET standing_capacity = NULL WHERE id = r.id;
  END LOOP;
END;
$rollback$;
