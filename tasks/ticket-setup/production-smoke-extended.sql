...
BEGIN
  BEGIN
    ...
    END;
    BEGIN
      PERFORM public.create_event_booking_v08(v_event,v_customer,1,'brand_site','seated',15,null,
        jsonb_set(v_guest,'{0,answers}','{}'),40);
      RAISE EXCEPTION 'Missing required answer unexpectedly accepted';
    EXCEPTION WHEN raise_exception THEN
      IF SQLERRM <> 'attendee_answer_required' THEN RAISE; END IF;
    END;
    BEGIN
    ...
    EXCEPTION WHEN raise_exception THEN
      IF SQLERRM <> 'attendee_count_mismatch' THEN RAISE; END IF;
    END;
    -- Reuse the already checked event only after prior holds have been cancelled.
    UPDATE public.bookings SET status='cancelled',hold_expires_at=NULL WHERE event_id=v_event;
    DELETE FROM public.booking_holds
    WHERE event_booking_id IN (SELECT id FROM public.bookings WHERE event_id=v_event) AND status='active';
    UPDATE public.events SET payment_mode='cash_only',booking_questions='[]' WHERE id=v_event;
    v_result := public.create_event_booking_v08(v_event,v_customer,1,'brand_site','seated',15,null,'[]',47);
    IF v_result->>'state'<>'confirmed' THEN RAISE EXCEPTION 'Ordinary pay-on-arrival booking failed: %',v_result; END IF;
    IF (SELECT attendees FROM public.bookings WHERE id=(v_result->>'booking_id')::uuid)<>'[]'::jsonb THEN RAISE EXCEPTION 'Pay-on-arrival guests unexpectedly required'; END IF;
    UPDATE public.event_ticket_types SET base_price=0 WHERE id=v_type;
    UPDATE public.events SET payment_mode='free' WHERE id=v_event;
    v_result := public.create_event_booking_v08(v_event,v_other,1,'brand_site','seated',15,null,'[]',0);
    IF v_result->>'state'<>'confirmed' THEN RAISE EXCEPTION 'Ordinary free booking failed: %',v_result; END IF;
    IF (SELECT attendees FROM public.bookings WHERE id=(v_result->>'booking_id')::uuid)<>'[]'::jsonb THEN RAISE EXCEPTION 'Free booking guests unexpectedly required'; END IF;
    v_checks := v_checks || '"required-answer rejection, ordinary free and pay-on-arrival without guest fields"'::jsonb;
