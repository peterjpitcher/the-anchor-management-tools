#!/usr/bin/env tsx
/**
 * Template loading diagnostics (read-only).
 *
 * Verifies that message template rows exist for the requested template types and that
 * the `get_message_template` RPC returns content for them.
 *
 * Safety:
...
const SCRIPT_NAME = 'test-template-loading'
