...
/**
 * Seed a single menu ingredient + price history row (Chef's Larder buttery mash).
 *
 * Safety:
 * - DRY RUN by default.
...
const SCRIPT_NAME = 'seed-chefs-larder-buttery-mash';
const RUN_MUTATION_ENV = 'RUN_SEED_CHEFS_LARDER_BUTTERY_MASH_MUTATION';
const ALLOW_MUTATION_ENV = 'ALLOW_SEED_CHEFS_LARDER_BUTTERY_MASH_MUTATION_SCRIPT';
const HARD_CAP = 10;
...
const rawFeatures = [
  ...
  'Enriched with milk and butter',
  'Suitable for Vegetarians',
  'Microwave 4 mins',
  'Real potato',
];
const rawLifestyleStatements = ['Suitable for Vegetarians'];
const manualDietaryFlags = [];
const rawDietaryFlags = [
  ...extractDietaryFlagsFromStatements([...rawFeatures, ...rawLifestyleStatements]),
  ...manualDietaryFlags,
];
const rawAllergens = ['milk'];

const ingredientPayload = {
  name: "Chef's Larder Buttery Mash Potato 2kg",
  description: 'Ready to cook potato mashed with milk and butter.',
  ...
  pack_cost: 4.09,
  portions_per_pack: 14,
  wastage_pct: 0,
  shelf_life_days: 180,
  allergens: normalizeAllergens(rawAllergens),
  dietary_flags: normalizeDietaryFlags(rawDietaryFlags),
  notes: [
    'Barcode: 5020379093675.',
    'Frozen pellets; contains 14 x 140g servings (approx. 10 pellets per serving).',
    'Always store frozen at -18°C or below. Do not refreeze once defrosted.',
    'Microwave: 140g in covered bowl, 3 min @1000W, stand 1 min, fluff with fork; add milk if needed.',
    'Hints: ideal for shepherd’s/fish pie toppings—add pellets directly before final bake.',
    'Warning: do not eat this product raw.',
    'Compliant with Department of Health salt targets (2017).',
    'Advice line: 0345 602 6002.',
  ].join(' '),
  is_active: true,
};
