...
async function checkCurrentSchema() {
  ...
  if (argv.includes('--confirm')) {
    throw new Error('check-schema-admin is strictly read-only; do not pass --confirm.')
  }

  console.log('Checking current database schema (admin)...\n')
...
void checkCurrentSchema().catch((error) => {
  markFailure('check-schema-admin failed.', error)
})
