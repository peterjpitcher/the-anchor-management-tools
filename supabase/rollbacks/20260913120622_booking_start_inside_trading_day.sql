-- Rollback for supabase/migrations/20260913120622_booking_start_inside_trading_day.sql.
--
-- Puts create_table_booking_core_v06 back to the definition production was running before the
-- trading-day fix: the body committed in 20260911133645_christmas_minimum_four.sql, body md5
-- 3c6603bd5310153422303e90e71b590b, 32366 characters. That md5 was read back from production
-- (pg_proc.prosrc) on 13 September 2026 immediately before the fix was applied, so this is the
-- exact text that was live, not a guess from an old file.
--
-- Running this restores the bug it replaced: a booking time before opening on a day that closes
-- after midnight goes back to being stored on the wrong calendar day, and refused as in_past when
-- made later the same day. New Year's Eve 2026 is the only date in the next four months where any
-- time qualifies.
--
-- The grants are restated because a CREATE OR REPLACE does not carry them and the guard tests in
-- christmas-minimum.test.ts and create-path-deposit.test.ts require the core to stay service_role
-- only, with the public and staff wrappers the application calls.
