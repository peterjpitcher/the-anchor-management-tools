-- Rollback for 20260911133645_christmas_minimum_four.sql
--
-- Puts the 6-guest Christmas minimum back. Both functions return to the production text captured
-- on 11 September 2026 (pg_get_functiondef md5 f8f7f2b84b4fdf9ff7cb5266416a21c8 for
-- create_table_booking_core_v06 and 988a2a5ddefd576aef392edaf70c67dc for
-- create_table_booking_v05), EXECUTE stays as it is, and the christmas-2026 booking period returns
-- to min_party_size = 6. Hand-run SQL, like everything in this folder.
--
-- It does not touch bookings. A Christmas booking for 4 or 5 guests taken while the lower minimum
-- was live stays exactly as it is. List them first and agree with the owner what happens to them:
--
--   SELECT id, booking_reference, booking_date, party_size, status
--     FROM public.table_bookings
--    WHERE booking_type = 'christmas' AND party_size < 6
...
-- 1. create_table_booking_core_v06, exactly as production held it on 11 September 2026.
...
-- 2. create_table_booking_v05, exactly as production held it on 11 September 2026.
...
-- 3. EXECUTE, unchanged either way.
...
-- 4. DATA: the christmas-2026 period back to 6, only if it is still at 4.

UPDATE public.booking_periods
   SET min_party_size = 6
 WHERE code = 'christmas-2026'
   AND min_party_size = 4;

-- 5. Prove the end state, or roll the rollback back.

DO $assert_christmas_minimum_rollback$
DECLARE
...
BEGIN
  -- Both entry points refuse below 6, and say so in the words staff are shown.
  FOREACH v_fn IN ARRAY ARRAY[v_core, v_v05] LOOP
    SELECT p.prosrc INTO v_src FROM pg_proc p WHERE p.oid = v_fn;
    IF position('IF p_party_size < 6 THEN' IN v_src) = 0
       OR position('''Christmas bookings are for 6 guests or more.''' IN v_src) = 0 THEN
      v_problems := v_problems || format('%s does not refuse Christmas parties under 6', v_fn);
    END IF;
  END LOOP;
  ...
  SELECT string_agg(p.oid::regprocedure::text, ', ') INTO v_stale
   ...
   WHERE n.nspname = 'public'
     AND p.prosrc LIKE '%Christmas bookings are for 4 guests or more.%';
  IF v_stale IS NOT NULL THEN
    v_problems := v_problems || format('still refusing Christmas parties under 4: %s', v_stale);
  END IF;
  ...
  -- The data change was undone.
  SELECT count(*), min(bp.min_party_size) INTO v_rows, v_min
    FROM public.booking_periods bp WHERE bp.code = 'christmas-2026';
  IF v_rows <> 1 OR v_min IS DISTINCT FROM 6 THEN
    v_problems := v_problems || format(
      'christmas-2026 booking period: %s row(s), min_party_size %s, expected 1 row at 6',
      v_rows, COALESCE(v_min::text, 'null'));
  END IF;

  IF cardinality(v_problems) > 0 THEN
    RAISE EXCEPTION USING
      MESSAGE = 'Christmas minimum not at 6: ' || array_to_string(v_problems, '; '),
      HINT = 'The whole rollback is undone, so nothing has changed.';
  END IF;
END
$assert_christmas_minimum_rollback$;
