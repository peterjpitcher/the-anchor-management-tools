-- Atomic capacity check: add pg_advisory_xact_lock to prevent concurrent slot oversell.
-- The table selection loop in create_table_booking_v05 had a race condition — between the
-- EXISTS check and the INSERT into booking_table_assignments, a concurrent transaction
-- could claim the same table. This migration adds a transaction-level advisory lock keyed
-- on the selected table ID immediately after selection, then re-verifies availability
-- while holding the lock before proceeding with the INSERT.
...
BEGIN
  ...
  -- Acquire a transaction-level advisory lock keyed on the selected table ID.
  -- This prevents concurrent transactions from claiming the same table between
  -- the EXISTS check above and the INSERT into booking_table_assignments below.
  -- The lock is automatically released when the transaction commits or rolls back.
  PERFORM pg_advisory_xact_lock(
    ('x' || substr(md5(v_selected_table.id::text), 1, 16))::bit(64)::bigint
  );

  -- Re-verify that the table is still available now that we hold the lock.
  -- A concurrent transaction may have inserted a conflicting assignment while
  ...
  IF EXISTS (
    ...
    -- Table was taken while we waited for the lock — no table available.
    RETURN jsonb_build_object('state', 'blocked', 'reason', 'no_table');
  END IF;
