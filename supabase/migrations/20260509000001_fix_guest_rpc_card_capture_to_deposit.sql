-- Fix create_table_booking_v05 (8-param guest/brand-site version):
-- Migration 20260508000006 added an advisory lock but kept the legacy card capture logic.
-- Migration 20260508000007 then dropped the card_captures table and card_capture_required
-- column, making the 8-param version fail for any Sunday lunch or 7+ person booking.
--
-- This migration rewrites the 8-param guest RPC to use pending_payment / payment_hold /
-- payments rows (matching the approach from the FOH 9-param version in 20260508000005).
-- The advisory lock from 20260508000006 is preserved.
...
BEGIN
  ...
  -- Find the smallest available table that fits the whole party.
  ...
  -- Acquire a transaction-level advisory lock keyed on the selected table ID to prevent
  -- concurrent transactions claiming the same table between the availability check above
  -- and the INSERT into booking_table_assignments below.
  PERFORM pg_advisory_xact_lock(
    ('x' || substr(md5(v_selected_table.id::text), 1, 16))::bit(64)::bigint
  );
  ...
  IF v_deposit_required THEN
    ...
    -- Insert a pending payments row so confirm_table_payment_v05 can find it
    -- immediately after the Stripe webhook fires.
