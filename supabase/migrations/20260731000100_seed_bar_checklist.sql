...
BEGIN
  ...
  INSERT INTO public.checklists (name, description, department, sort_order, is_active)
  VALUES ('Bar Periodic', 'Bar tasks due every few days, plus weekly pool-table tasks.', 'bar', 5, true)
  ...
  ----------------------------------------------------------------------------
  -- 3. Bar Opening (daily items 1-20; typos corrected: Cutlery, Condiments)
  ...
  VALUES
    (v_opening, 'Switch on TVs, fruit machine and jukebox', 1, 'calendar', 'daily', 'open', true, false, 1),
    (v_opening, 'Hoover carpet area if needed', 2, 'calendar', 'daily', 'open', true, false, 1),
    (v_opening, 'Spot Mop throughout floors (blue bucket)', 3, 'calendar', 'daily', 'open', true, false, 1),
    (v_opening, 'Check inside and outside areas are clean and tidy and empty ashtrays', 4, 'calendar', 'daily', 'open', true, false, 1),
    (v_opening, 'Reset tables so they''re all ready to the right standard', 5, 'calendar', 'daily', 'open', true, false, 1),
    (v_opening, 'Menus are in the holder ready', 6, 'calendar', 'daily', 'open', true, false, 1),
    (v_opening, 'Whatsapp Pete if you need more menus (main, Sunday, Kids, etc).', 7, 'calendar', 'daily', 'open', true, false, 1),
    (v_opening, 'Load ice bucket', 8, 'calendar', 'daily', 'open', true, false, 1),
    ...
    (v_opening, 'Check the till for bookings and place chalkboards and highchairs', 18, 'calendar', 'daily', 'open', true, false, 1),
    (v_opening, 'Water outside plants in the back garden (one watering can per pot)', 19, 'calendar', 'daily', 'open', true, false, 1),
    (v_opening, 'Sweep the stones', 20, 'calendar', 'daily', 'open', true, false, 1);

  -- Opening item 21: seasonal (Autumn/Winter only), 01 Sep - 31 Mar
  INSERT INTO public.checklist_task_templates
    (checklist_id, title, sort_order, schedule_kind, freq, anchor, is_spot_checkable,
     season_start, season_end, is_active, version)
  VALUES
    (v_opening, 'Load candles into holders and light at open', 21, 'calendar', 'daily', 'open', true, '09-01', '03-31', false, 1);

  -- Weekly menu swaps at opening (Sun=0, Tue=2).
  INSERT INTO public.checklist_task_templates
    (checklist_id, title, sort_order, schedule_kind, freq, by_weekday,
     anchor_date, anchor, is_spot_checkable, is_active, version)
  VALUES
    (v_opening, 'Switch weekday menus for Sunday menus from caddy drawer', 22, 'calendar', 'weekly', ARRAY[0],
     DATE '2026-08-09', 'open', true, false, 1),
    (v_opening, 'Switch Sunday menus for weekday menus from caddy drawer', 23, 'calendar', 'weekly', ARRAY[2],
     DATE '2026-08-11', 'open', true, false, 1);
  ...
  INSERT INTO public.checklist_task_templates
    (checklist_id, title, sort_order, schedule_kind, freq, anchor, is_spot_checkable, is_active, version)
  VALUES
    (v_closing, 'Clean all tables and remove beer mats (throw out dirty mats)', 1, 'calendar', 'daily', 'close', true, false, 1),
    ...
    (v_closing, 'Put all mayonnaise bottles into the fridge', 22, 'calendar', 'daily', 'close', true, false, 1);
  ...
  VALUES
    (v_cleaning, 'Cleaning check',
     'Regular Cleaning Check Points: Toilet (including toilet rolls and paper towels in holders, handwash topped up, sinks and toilets sprayed and wiped if required, and floor mopped if needed), Jukebox, Pool Table, Fruit Machine, Till and door push plates.',
     1, 'calendar', 'daily', 'every', 2, true, false, 1);

  ----------------------------------------------------------------------------
  -- 6. Bar Periodic (floating cadence, intervals from decisions 27 and 33)
  --    Most are floating / anchor=anytime / spot-checkable. first_due_on staggered
  --    across 2026-08-01 .. 2026-08-06 so they do not all fall due on day one.
  --    Machine cleaning is calendar-based every other day (decision 28).
  ----------------------------------------------------------------------------
  INSERT INTO public.checklist_task_templates
  ...
  VALUES
    (v_periodic, 'Wipe chairs and table legs',                    1, 'floating', 'anytime', 3, 2, DATE '2026-08-01', true, false, 1),
    ...
    (v_periodic, 'Clean glass racks',                             4, 'floating', 'anytime', 3, 1, DATE '2026-08-02', true, false, 1),
    (v_periodic, 'Clean display bottles and shelves',             5, 'floating', 'anytime', 5, 2, DATE '2026-08-03', true, false, 1),
    (v_periodic, 'Clean window seals',                            6, 'floating', 'anytime', 2, 1, DATE '2026-08-03', true, false, 1),
    (v_periodic, 'Rotate fridge and bottle stock',               10, 'floating', 'anytime', 3, 1, DATE '2026-08-05', true, false, 1);
  ...
  -- Clean the machines on a fixed every-other-day cadence.
  INSERT INTO public.checklist_task_templates
    (checklist_id, title, sort_order, schedule_kind, freq, freq_interval,
     anchor_date, anchor, is_spot_checkable, is_active, version)
  VALUES
    (v_periodic, 'Clean TVs, fruit machine and jukebox', 13, 'calendar', 'daily', 2,
     DATE '2026-08-05', 'anytime', true, false, 1);

  -- Weekly pool-table free-play changes.
  INSERT INTO public.checklist_task_templates
    (checklist_id, title, sort_order, schedule_kind, freq, by_weekday,
     anchor_date, anchor, is_spot_checkable, is_active, version)
  VALUES
    (v_periodic, 'Turn the pool table to free play', 14, 'calendar', 'weekly', ARRAY[4],
     DATE '2026-08-06', 'anytime', true, false, 1),
    (v_periodic, 'Turn the pool table off free play', 15, 'calendar', 'weekly', ARRAY[5],
     DATE '2026-08-07', 'anytime', true, false, 1);
END $$;
