-- Add joinable table configuration and multi-table allocation support for table bookings.

CREATE TABLE IF NOT EXISTS public.table_join_links (
  ...
  CONSTRAINT table_join_links_pair_check CHECK (table_id <> join_table_id),
  CONSTRAINT table_join_links_order_check CHECK (table_id < join_table_id)
);

CREATE INDEX IF NOT EXISTS idx_table_join_links_join_table_id
  ON public.table_join_links (join_table_id);
