-- Fix confirm_table_payment_v05 to handle confirmed bookings with pending payment_status.
-- Previously, if a booking was already 'confirmed' (e.g. created by a manager override)
-- but had payment_status = 'pending', the RPC would record the payment but NOT update
-- payment_status to 'completed', causing the deposit link to remain visible indefinitely.
...
BEGIN
  ...
  -- Booking was already confirmed (e.g. manager-created) but payment_status was pending.
  -- Mark payment as completed and consume the token so the link won't reappear.
