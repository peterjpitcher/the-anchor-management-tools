-- Allow FOH staff to book any time before kitchen/pub closes, bypassing the
-- 30-minute pre-close buffer that protects online/self-service customers.
--
-- Strategy: drop the old 8-parameter signature and recreate with a new
-- p_bypass_cutoff parameter (DEFAULT false). Existing callers without the
-- parameter continue to get the 30-minute buffer; the FOH API passes true.
...
BEGIN
  ...
  IF v_purpose = 'food' OR COALESCE(p_sunday_lunch, false) THEN
    ...
    -- 30-minute pre-close buffer: skipped for FOH (staff booking on behalf of a customer)
