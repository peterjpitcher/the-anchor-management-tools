...
-- Log any remaining invalid phone numbers before applying constraints
DO $$
DECLARE
  invalid_count INTEGER;
  invalid_customers RECORD;
  invalid_bank_count INTEGER;
  invalid_bank_details RECORD;
BEGIN
  -- Count invalid customer phone numbers
  ...
  IF invalid_count > 0 THEN
    RAISE NOTICE 'Found % customers with invalid phone numbers that could not be auto-fixed:', invalid_count;
    
    FOR invalid_customers IN 
    ...
    LOOP
      RAISE NOTICE 'Customer % % (ID: %): %', 
        invalid_customers.first_name, 
        invalid_customers.last_name, 
        invalid_customers.id, 
        invalid_customers.mobile_number;
    END LOOP;
    
    -- Option 1: Set invalid numbers to NULL (uncomment to use)
    -- UPDATE customers SET mobile_number = NULL 
    -- WHERE mobile_number IS NOT NULL 
    --   AND mobile_number !~* '^(\+?44|0)?[0-9]{10,11}$';
    
    -- Option 2: Fail the migration (default behavior)
    RAISE EXCEPTION 'Cannot apply constraints due to invalid phone numbers. Please fix manually or uncomment the NULL update above.';
  END IF;

  -- Count invalid bank details
  SELECT COUNT(*) INTO invalid_bank_count
  FROM employee_financial_details
  WHERE (bank_account_number IS NOT NULL AND bank_account_number !~* '^[0-9]{8}$')
    OR (bank_sort_code IS NOT NULL AND bank_sort_code !~* '^[0-9]{2}-?[0-9]{2}-?[0-9]{2}$');
  
  IF invalid_bank_count > 0 THEN
    RAISE NOTICE 'Found % employee financial records with invalid bank details:', invalid_bank_count;
    
    FOR invalid_bank_details IN 
      SELECT efd.employee_id, efd.bank_account_number, efd.bank_sort_code,
             e.first_name, e.last_name
      FROM employee_financial_details efd
      JOIN employees e ON e.employee_id = efd.employee_id
      WHERE (efd.bank_account_number IS NOT NULL AND efd.bank_account_number !~* '^[0-9]{8}$')
        OR (efd.bank_sort_code IS NOT NULL AND efd.bank_sort_code !~* '^[0-9]{2}-?[0-9]{2}-?[0-9]{2}$')
      LIMIT 10
    LOOP
      RAISE NOTICE 'Employee % % (ID: %): Account: %, Sort Code: %', 
        invalid_bank_details.first_name, 
        invalid_bank_details.last_name, 
        invalid_bank_details.employee_id,
        invalid_bank_details.bank_account_number,
        invalid_bank_details.bank_sort_code;
    END LOOP;
    
    -- Option: Set invalid bank details to NULL (uncomment to use)
    -- UPDATE employee_financial_details 
    ...
    -- WHERE bank_sort_code IS NOT NULL AND bank_sort_code !~* '^[0-9]{2}-?[0-9]{2}-?[0-9]{2}$';
    
    RAISE EXCEPTION 'Cannot apply constraints due to invalid bank details. Please fix manually or uncomment the NULL updates above.';
  END IF;
END $$;
