-- Add date_tbd boolean column to private_bookings so TBD bookings are tracked
-- via a real column instead of relying solely on internal_notes convention.

-- 1. Add the column
ALTER TABLE public.private_bookings
  ADD COLUMN date_tbd BOOLEAN NOT NULL DEFAULT false;

-- 2. Backfill existing TBD bookings from the notes convention
UPDATE public.private_bookings
  SET date_tbd = true
  WHERE internal_notes LIKE '%Event date/time to be confirmed%';

-- 3. Update calculate_balance_due_date trigger to respect date_tbd
...
-- 4. Update create_private_booking_transaction RPC to accept date_tbd
