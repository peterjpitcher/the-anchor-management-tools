-- Fix: correct overlap check in create_table_booking_v05_core_legacy.
--
-- Problem: The function only excluded 'cancelled' bookings from the overlap
-- check.  Bookings in status 'no_show', or where the guest has already left
-- (left_at IS NOT NULL), were still treated as occupying the table.  This
-- caused the FOH event-booking path (which routes through
-- create_event_table_reservation_v05_legacy → create_table_booking_v05_core
-- → ...→ this function) to return 'no_table' even when tables were free.
--
-- Fix: both the single-table SELECT and the available_tables CTE now use
--   AND tb.status NOT IN ('cancelled', 'no_show') AND tb.left_at IS NULL
-- matching the logic in the canonical create_table_booking_v05 (10-param).
...
BEGIN
  ...
  -- Single-table search: exclude cancelled, no_show, and already-departed bookings.
  SELECT
    t.id,
  ...
  ELSE
    -- Combo search: same corrected overlap check in the available_tables CTE.
