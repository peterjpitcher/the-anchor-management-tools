...
BEGIN
  ...
  WITH input_rows AS (
    SELECT
      ...
      COALESCE((item->>'sms_opt_in')::boolean, true) AS sms_opt_in,
      COALESCE(NULLIF(item->>'sms_status', ''), 'active') AS sms_status,
      COALESCE((item->>'marketing_sms_opt_in')::boolean, false) AS marketing_sms_opt_in,
      COALESCE((item->>'whatsapp_opt_in')::boolean, true) AS whatsapp_opt_in,
      COALESCE(NULLIF(item->>'whatsapp_status', ''), 'active') AS whatsapp_status,
      COALESCE((item->>'marketing_whatsapp_opt_in')::boolean, false) AS marketing_whatsapp_opt_in,
      COALESCE((item->>'marketing_email_opt_in')::boolean, false) AS marketing_email_opt_in
  ...
  inserted AS (
    ...
    SELECT
      ...
      v.email,
      v.sms_opt_in,
      CASE WHEN v.sms_opt_in THEN v.sms_status ELSE 'opted_out' END,
      v.marketing_sms_opt_in,
      v.whatsapp_opt_in,
      CASE WHEN v.whatsapp_opt_in THEN v.whatsapp_status ELSE 'opted_out' END,
      v.marketing_whatsapp_opt_in,
      v.marketing_email_opt_in
    FROM valid_rows v
    WHERE NOT EXISTS (
      SELECT 1
