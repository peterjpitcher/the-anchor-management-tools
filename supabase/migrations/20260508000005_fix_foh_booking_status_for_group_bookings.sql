-- Fix create_table_booking_v05: group bookings (party_size >= 7, non-Sunday) were being
-- set to 'pending_card_capture' instead of 'pending_payment'. Card capture is retired;
-- the correct flow for FOH group bookings is:
--   • deposit_method = 'cash'         → booking ends up 'confirmed' (cash taken in person)
--   • deposit_method = 'payment_link' → booking stays 'pending_payment' (customer pays online)
--
-- Changes:
-- 1. v_card_capture_required now controls whether a deposit hold is needed, but the
--    booking status is always 'pending_payment' (not 'pending_card_capture') when a deposit
--    is required.
-- 2. A 'payment_hold' is inserted instead of a 'card_capture_hold' / 'card_captures' record.
-- 3. A pending 'table_deposit' payments row is inserted so that record_table_cash_deposit_v05
--    and confirm_table_payment_v05 can find it immediately after creation.
-- 4. The returned JSONB uses 'pending_payment' for the state field, so the FOH route's
--    existing deposit-method branching (lines 1121-1217 of route.ts) works correctly.

DROP FUNCTION IF EXISTS public.create_table_booking_v05(
  uuid, date, time without time zone, integer, text, text, boolean, text, boolean
);
...
BEGIN
  ...
  -- Deposit is required for Sunday lunch or group bookings (7-20 people).
  -- Card capture is retired: always use pending_payment, not pending_card_capture.
  ...
  INSERT INTO public.table_bookings (
    ...
    p_party_size,
    v_hold_expires_at,
    false,  -- card_capture_required is always false; card capture is retired
  ...
  IF v_deposit_required THEN
    -- Insert a payment_hold (not a card_capture_hold — card capture is retired).
    ...
    -- Insert a pending payments row so record_table_cash_deposit_v05 /
    -- confirm_table_payment_v05 can find it immediately after creation.
...
REVOKE ALL ON FUNCTION public.create_table_booking_v05(uuid, date, time without time zone, integer, text, text, boolean, text, boolean) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.create_table_booking_v05(uuid, date, time without time zone, integer, text, text, boolean, text, boolean) TO service_role;
