-- Manual event-interest recipients and event_type persistence in event transactions.

CREATE TABLE IF NOT EXISTS public.event_interest_manual_recipients (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  event_id uuid NOT NULL REFERENCES public.events(id) ON DELETE CASCADE,
  customer_id uuid NOT NULL REFERENCES public.customers(id) ON DELETE CASCADE,
  created_at timestamptz NOT NULL DEFAULT now(),
  CONSTRAINT event_interest_manual_recipients_event_customer_unique UNIQUE (event_id, customer_id)
);

CREATE INDEX IF NOT EXISTS idx_event_interest_manual_recipients_event_created
  ON public.event_interest_manual_recipients (event_id, created_at DESC);

CREATE INDEX IF NOT EXISTS idx_event_interest_manual_recipients_customer
  ON public.event_interest_manual_recipients (customer_id, created_at DESC);
...
BEGIN
  UPDATE events SET
    ...
    booking_mode = COALESCE(NULLIF(p_event_data->>'booking_mode', ''), booking_mode),
    event_type = COALESCE(NULLIF(TRIM(p_event_data->>'event_type'), ''), event_type),
