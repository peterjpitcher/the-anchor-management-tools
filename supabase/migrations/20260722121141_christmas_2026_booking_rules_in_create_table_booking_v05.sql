-- Christmas 2026 booking rules inside create_table_booking_v05.
--
-- RECONCILIATION RECORD. This change was applied straight to production
-- (tfcasgxopxegwrabvwat) on 2026-07-22 with no migration file committed
-- ("The database side is already applied", commit f8a24720). Production recorded
-- version 20260722121141, the repo never had it, and the mismatch blocked
-- `supabase db push` for every later migration. Worse, no local migration
-- contained the Christmas rules at all, so rebuilding from migrations would have
-- silently produced a booking engine without them.
--
-- The body below is the live definition, captured with `supabase db dump` on
-- 2026-07-23 and pasted verbatim rather than retyped, so it cannot drift from
-- what production actually runs.
--
-- What it does, for readers who did not see the original change: Christmas
-- bookings (10 Nov to 20 Dec 2026) always take a deposit whatever the party size,
-- rather than the usual 10-or-more rule. p_booking_purpose accepts 'christmas',
-- which maps to 'food' internally so kitchen hours, duration and pacing still
-- apply, and stamps booking_type = 'christmas'.
--
-- Depends on migration 20260722121116, which adds the 'christmas' enum value.
-- PostgreSQL cannot use a new enum value in the transaction that creates it,
-- which is why the two are separate migrations.

CREATE OR REPLACE FUNCTION "public"."create_table_booking_v05"("p_customer_id" "uuid", "p_booking_date" "date", "p_booking_time" time without time zone, "p_party_size" integer, "p_booking_purpose" "text" DEFAULT 'food'::"text", "p_notes" "text" DEFAULT NULL::"text", "p_sunday_lunch" boolean DEFAULT false, "p_source" "text" DEFAULT 'brand_site'::"text", "p_bypass_cutoff" boolean DEFAULT false, "p_deposit_waived" boolean DEFAULT false, "p_bypass_pacing" boolean DEFAULT false, "p_high_chair_count" integer DEFAULT 0, "p_outside_seating" boolean DEFAULT false) RETURNS "jsonb"
