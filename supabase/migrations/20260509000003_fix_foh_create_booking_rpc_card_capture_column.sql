-- Fix create_table_booking_v05 (9-param FOH version):
-- Migration 20260508000005 updated the FOH function to use pending_payment/payment_hold/payments
-- (retiring card capture), but accidentally left card_capture_required in the INSERT column list
-- with a hardcoded false value. Migration 20260508000007 then dropped that column, causing every
-- FOH booking creation to fail with:
--   ERROR: column "card_capture_required" of relation "table_bookings" does not exist
--
-- Fix: remove card_capture_required from the INSERT column list, the INSERT values list,
-- and the RETURN jsonb. All other logic (payment_hold, pending_payment, payments row) is correct.
