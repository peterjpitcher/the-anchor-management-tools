-- Fix: remove all references to dropped card-capture infrastructure from
-- create_event_table_reservation_v05.
--
-- When migration 20260508000007 dropped the card-capture infrastructure it
-- did NOT update create_event_table_reservation_v05, leaving four dead
-- references that cause every FOH event-booking call to fail silently:
--
--   1. UPDATE public.card_captures  (table dropped — throws relation-not-found)
--      → caught by EXCEPTION WHEN OTHERS → returns no_table
--
--   2. card_capture_required = false  ×2  (column dropped — throws column-not-found)
--      One in the idempotency UPDATE path, one in the new-booking UPDATE path.
--
--   3. NOT IN ('confirmed', 'pending_card_capture')  (state check)
--      Should be 'pending_payment' following the rename in migration 003.
--      Only matters for group bookings (7+ people) but should be consistent.
--
-- This migration replaces the function with a clean version that:
--   • Removes UPDATE public.card_captures entirely
--   • Removes card_capture_required from both UPDATE statements
--   • Replaces pending_card_capture with pending_payment in the state check
--   • Preserves all other logic from migration 20260509000009 (overlap fixes)
...
BEGIN
  ...
  -- Idempotency: if we already have a linked (non-cancelled) table booking, return it.
  SELECT tb.id
  ...
  BEGIN
    ...
    -- FIX: pending_card_capture renamed to pending_payment in migration 003
    ...
    -- Release any payment_hold artifacts created by the underlying function.
    -- (card_capture_hold and card_captures were removed in migration 20260508000007)
    ...
    -- FIX: card_capture_required column dropped in migration 20260508000007
    UPDATE public.table_bookings
