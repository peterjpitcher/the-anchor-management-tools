-- Fix: confirm_table_payment_v05 was returning 'already_confirmed' without
-- updating table_bookings.payment_status when a booking was already in
-- 'confirmed' status (e.g. manager-created bookings). This meant the payment
-- row was marked succeeded in Stripe but the booking's payment_status stayed
-- 'pending', the deposit link remained visible, and the confirmation SMS was
-- never sent.
--
-- Fix: in the already-confirmed branch, update payment_status → 'completed',
-- consume the guest token, and return state='confirmed' so the webhook
-- dispatches SMS and analytics as normal.
...
BEGIN
  ...
  -- Booking was already confirmed (e.g. manager-created) — the payment row is
  -- now succeeded; update payment_status and consume the guest token so the
  -- deposit link disappears and the confirmation SMS fires.
