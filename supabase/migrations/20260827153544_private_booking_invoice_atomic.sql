-- Phase 2: create_private_booking_invoice_atomic
--
-- One transaction that turns a confirmed private booking into an invoice.
-- Modelled on convert_quote_to_invoice_atomic, including its base36 invoice
-- number encoding, so the two read the same way.
--
-- Why this exists rather than four separate app-layer calls: creating the
-- invoice, copying payments, linking the booking and sending the email were
-- originally four commits. That let two clicks create two invoices, a failure
-- mid-copy leave a half-populated invoice, and a retry double-count payments.
--
-- Divergences from the quote precedent, both deliberate:
--   1. It RETURNS the existing invoice instead of raising when the booking is
--      already invoiced. The button generates AND sends in one click, so a
--      failed send has to be retryable without burning a second number.
--   2. It copies payments. Quotes have none.
--
-- Line items and header totals are computed by the TypeScript mapper
-- (src/lib/private-bookings/invoice-mapper.ts) and passed in, because the
-- readable-description and discount-representation logic belongs somewhere
-- unit-testable. This function is the transactional and arithmetic guard: it
-- asserts the total it was handed equals get_booking_gross_total for the
-- booking, and refuses to write anything if it does not.
