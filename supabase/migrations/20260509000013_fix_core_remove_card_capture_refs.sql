-- Fix: remove dead card-capture references from create_table_booking_v05_core.
--
-- Migration 20260425000000 added deposit logic for group bookings (7+ people)
-- to create_table_booking_v05_core, but still referenced two pieces of
-- dropped card-capture infrastructure:
--
--   1. UPDATE public.card_captures  (table dropped in 20260508000007)
--      → throws relation-not-found exception at runtime
--      → caught by EXCEPTION WHEN OTHERS in create_event_table_reservation_v05
--      → all event bookings with 7+ people fail silently with reason 'no_table'
--
--   2. card_capture_required = false  (column dropped in 20260508000007)
--      → throws column-not-found exception (same silent failure mode)
--
-- This migration replaces the function with a clean version that:
--   • Removes UPDATE public.card_captures entirely
--   • Removes card_capture_required from the UPDATE public.table_bookings
--   • Preserves all other logic unchanged
