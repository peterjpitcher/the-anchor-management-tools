...
ADD COLUMN IF NOT EXISTS default_is_free BOOLEAN DEFAULT true,
...
COMMENT ON COLUMN event_categories.meta_description IS 'Default SEO meta description template';
...
COMMENT ON COLUMN event_categories.default_price IS 'Default ticket price';
COMMENT ON COLUMN event_categories.default_is_free IS 'Whether events in this category are typically free';
COMMENT ON COLUMN event_categories.default_booking_url IS 'Default external booking URL';
COMMENT ON COLUMN event_categories.default_performer_type IS 'Default performer type (Person, MusicGroup, etc)';
