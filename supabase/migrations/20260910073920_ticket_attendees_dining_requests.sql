-- Preserve existing dining requests alongside named ticket guests in one transaction.
SET lock_timeout = '5s';
CREATE OR REPLACE FUNCTION public.create_event_booking_with_attendees_and_requests_v01(
  p_event_id uuid,
  p_customer_id uuid,
  p_seats integer,
...
BEGIN
  ...
  v_result := public.create_event_booking_v08(
    p_event_id, p_customer_id, p_seats, p_source, p_seating_preference,
    p_payment_hold_minutes, p_ticket_selections, p_attendees, p_expected_total
  );

  -- Conflicts can contain the ID of an EXISTING booking. Never change it.
  -- Only these explicit states are new successful creates in v08.
...
REVOKE ALL ON FUNCTION public.create_event_booking_with_attendees_and_requests_v01(uuid, uuid, integer, text, text, integer, jsonb, text, boolean, jsonb, numeric) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.create_event_booking_with_attendees_and_requests_v01(uuid, uuid, integer, text, text, integer, jsonb, text, boolean, jsonb, numeric) TO service_role;
COMMENT ON FUNCTION public.create_event_booking_with_attendees_and_requests_v01(uuid, uuid, integer, text, text, integer, jsonb, text, boolean, jsonb, numeric) IS 'Service-only atomic event creation with guest snapshots and unconfirmed dining and early-arrival requests.';
