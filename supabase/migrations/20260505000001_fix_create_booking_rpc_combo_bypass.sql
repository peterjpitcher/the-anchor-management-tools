-- Fix create_table_booking_v05: merge joined-combo search with p_bypass_cutoff.
--
-- Problem: two incompatible overloads exist in the DB:
--   • 8-param (from 20260503000008): has joined-combo search, but no p_bypass_cutoff →
--     public API uses this and CAN book large joined parties.
--   • 9-param (from 20260301130000): has p_bypass_cutoff, but NO combo search →
--     FOH API passes p_bypass_cutoff=true, hits this version, and CANNOT book large
--     parties (returns no_table for any party size > single table capacity).
--
-- Fix: drop both, create a single 9-param version with combo search AND bypass_cutoff.
