-- Lets guest campaigns send at weekends while business campaigns stay on weekdays.
--
-- The Monday to Friday window was set for a corporate audience, where a Saturday morning
-- email lands in an inbox nobody opens until Monday and looks careless when they do. Applied
-- to guests it is simply wrong: a Saturday morning is one of the better times to reach
-- somebody about booking Christmas dinner, because they are not at work.
--
-- One window, two day sets, chosen by the campaign's audience. The hours stay shared, since
-- "not before 9am and not after 6pm" is good manners for anybody.

BEGIN;

ALTER TABLE public.marketing_settings
  ADD COLUMN IF NOT EXISTS customer_send_days smallint[] NOT NULL DEFAULT '{1,2,3,4,5,6,7}';

COMMENT ON COLUMN public.marketing_settings.send_days IS
  'Days business campaigns may send, ISO day numbers. Weekdays only by default: a corporate '
  'inbox does not want a weekend marketing email.';
COMMENT ON COLUMN public.marketing_settings.customer_send_days IS
  'Days guest campaigns may send. All seven by default, because a weekend morning is a good '
  'time to reach somebody who is not at work.';

CREATE OR REPLACE FUNCTION public.marketing_send_window_open(p_audience_type text DEFAULT 'business')
RETURNS boolean
LANGUAGE sql
STABLE
...
AS $$
  SELECT COALESCE(
    (
      SELECT
        EXTRACT(ISODOW FROM timezone('Europe/London', now()))::smallint = ANY (
          CASE WHEN p_audience_type = 'customer' THEN s.customer_send_days ELSE s.send_days END
        )
...
-- Promote: the window is now decided per campaign, so a guest campaign due on a Saturday is
-- promoted while a business one waits for Monday.
...
BEGIN
  ...
  LOOP
    CONTINUE WHEN NOT public.marketing_send_window_open(v_campaign.audience_type);
...
REVOKE ALL ON FUNCTION public.marketing_send_window_open(text) FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.promote_due_marketing_campaigns() FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.marketing_send_window_open(text) TO service_role;
GRANT EXECUTE ON FUNCTION public.promote_due_marketing_campaigns() TO service_role;

-- Claim: the same per-campaign decision. Without this the guest campaign would be promoted on
-- a Saturday and then never claimed, because the old call defaulted to the business day set.
