-- Exclude communal-event tables from the new-booking table allocator.
--
-- Policy: a table used for a communal event cannot be shared with a food (table) booking.
-- The DB trigger enforce_booking_table_assignment_integrity_v05 already rejects any food
-- assignment on a table with active communal seats. But create_table_booking_v05's allocator
-- (Step 1 single table, Step 2 joined-table combos) only excluded tables with overlapping
-- *bookings* — it had no communal check. So it could pick a communal table and then fail at
-- insert time (the trigger raises table_assignment_communal_overlap), returning a generic error.
--
-- This recreates the function with the SAME communal-overlap exclusion the trigger enforces,
-- added to both allocation steps, so the allocator never selects an event table. Everything
-- else is unchanged. Idempotent (CREATE OR REPLACE).
...
BEGIN
  ...
  WHERE COALESCE(t.is_bookable, true) = true
    ...
    -- A table held for a communal event cannot be shared with a food booking.
    AND NOT EXISTS (
      SELECT 1
  ...
  -- Edit 2 (Migration B): replaced legacy "Sunday lunch OR 7-20 group" rule
  -- with the new 10+ threshold that respects p_deposit_waived. Card capture
  -- is retired: always use pending_payment, not pending_card_capture.
  ...
  -- (kept for traceability — the new rule above already accounts for waiver)
  ...
  -- Edit 1 (Migration B): sunday_preorder_cutoff_at only set for legacy
  -- Sunday-lunch bookings — non-Sunday rows leave the column NULL.
  ...
  -- Edit 3 (Migration B): deposit_amount calc UNCHANGED (party_size * 10).
  ...
  IF v_deposit_required THEN
    -- Insert a payment_hold (card capture is retired).
