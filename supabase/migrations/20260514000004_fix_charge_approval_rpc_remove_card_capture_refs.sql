-- Fix get_charge_request_approval_preview_v05 and decide_charge_request_v05:
-- remove references to the dropped card_captures table.
--
-- Migration 20260508000007 dropped the card_captures table with CASCADE,
-- but these two charge-approval RPC functions (from 20260420000013) still
-- query card_captures to find a stripe_payment_method_id.  Every call to
-- either function now throws "relation public.card_captures does not exist",
-- which propagates to the approval page as an opaque "invalid_token" error.
--
-- Fix: remove the card_captures query from both functions.  stripe_payment_method_id
-- is now always NULL (payment_method_available = false).  The approval page
-- already renders a warning when payment_method_available is false, so no
-- UI changes are required.  Managers can still approve/waive; the charge
-- attempt will fail at execution time if no Stripe customer exists, and the
-- page surfaces that clearly.
