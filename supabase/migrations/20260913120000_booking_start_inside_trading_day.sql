...
-- so on a night that closes after midnight it landed a day early: a 00:15 booking for New Year's
-- Eve 2026 (special hours 12:00 to 01:00) was stored at 00:15 on the morning of 31 December.
-- The hours check in the same function already reads that time correctly, as the far end of the
-- night, and check_table_availability_v06 offers those slots, so the two disagreed. A booking
-- made after 00:15 on the 31st was also refused as 'in_past' for the same reason.
--
-- Only the start (and the end and table windows built from it) changes, and only on a day whose
-- close is earlier than its opening time. The two such days in the next four months are
-- 31 October 2026 (closes 00:00, where no time qualifies) and 31 December 2026. No stored
-- booking is affected: a read-only check over every booking from the last 30 days onwards
-- returned none on a late-closing day before opening.
--
-- Based on the definition applied as christmas_minimum_four
-- (supabase/migrations/20260911133645_christmas_minimum_four.sql), whose body matches production
-- byte for byte (md5 3c6603bd5310153422303e90e71b590b, 32,366 bytes).
...
BEGIN
  ...
  -- The hours come first, because on a night that closes after midnight they decide which
  -- calendar day the booking's start falls on, and the in-past check below needs that answer.
  SELECT
    bh.day_of_week,
  ...
  -- A close earlier than the opening time is a night that runs past midnight: New Year's Eve
  -- 2026 is 12:00 to 01:00, and that 01:00 is 1 January. A time before opening on such a day is
  -- the far end of the same night, so 00:15 on 31 December is a quarter past midnight on
  -- 1 January, not the previous morning. This is the same test the hours check below applies to
  -- the booking time (v_pub_booking_minutes + 1440), and the same rule the application uses in
  -- serviceInstantFor (src/lib/business-hours/trading-day.ts). Every day that closes before
  -- midnight keeps the calendar date exactly as before.
  IF v_hours_row.closes <= v_hours_row.opens AND p_booking_time < v_hours_row.opens THEN
    v_booking_start_local := v_booking_start_local + INTERVAL '1 day';
  END IF;
...
-- ---------------------------------------------------------------------------
-- EXECUTE, restated rather than inherited.
--
-- A CREATE OR REPLACE keeps the grants the function already holds, so these two statements
-- change nothing in production: the core is service_role only, and the public and staff wrappers
-- are what the application calls. They are here so the end state is stated rather than inherited,
-- and so a rebuilt database comes out the same.
-- ---------------------------------------------------------------------------
