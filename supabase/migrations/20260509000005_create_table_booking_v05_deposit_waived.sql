...
-- When a manager/super_admin waives the deposit, the booking is confirmed immediately
-- (status = 'confirmed', hold_expires_at = NULL) regardless of party size or booking type.
-- Uses CREATE OR REPLACE — all existing callers are unaffected because the new parameter
