...
ALTER TABLE public.tables
  ...
  ADD COLUMN IF NOT EXISTS event_priority     integer NOT NULL DEFAULT 100,
...
COMMENT ON COLUMN public.tables.priority IS
  'Fill order for TABLE BOOKINGS within a capacity band. Lower wins. Set by the manager; never derived from the name.';
COMMENT ON COLUMN public.tables.event_priority IS
  'Fill order for EVENT communal seating. Deliberately the reverse of priority: events take the bar tables first so the Dining Room stays free for diners.';
...
--
-- TWO orders, deliberately opposed (owner, 2026-07-27):
--
--   priority        TABLE BOOKINGS. The Dining Room comes FIRST, because that is where diners
...
--
--   event_priority  EVENT communal seating. The reverse: events take the bar tables first and the
--                   Dining Room LAST, so a quiz or bingo night does not consume the dining space
--                   that food bookings need.
--
...
-- ---------------------------------------------------------------------------
UPDATE public.tables SET priority =  10, event_priority =  60, min_party_size = 1
  WHERE id = '39350c06-d5ea-4cea-a742-9ea78ebc0557';  -- Dining Room 4a
UPDATE public.tables SET priority =  20, event_priority =  70, min_party_size = 1
  WHERE id = 'f16044f7-8dcf-4403-8e89-02992fdc9532';  -- Dining Room 4b
UPDATE public.tables SET priority =  30, event_priority =  80, min_party_size = 4
  WHERE id = '5deb3b97-1f18-4ee7-97c9-887b47ff504e';  -- Dining Room 6a
UPDATE public.tables SET priority =  40, event_priority =  90, min_party_size = 4
  WHERE id = 'eca30e1a-9000-410a-97f3-c7bda2ed538b';  -- Dining Room 6b
UPDATE public.tables SET priority =  50, event_priority = 100, min_party_size = 4
  WHERE id = 'fc306a12-0cb2-4692-bf3f-cfb89466abb6';  -- Dining Room 6c
UPDATE public.tables SET priority =  60, event_priority =  40, min_party_size = 1, step_free = false
  WHERE id = '37d61f34-0eed-4a97-9e8c-aa868fdfe779';  -- Small Bay
UPDATE public.tables SET priority =  70, event_priority =  50, min_party_size = 4
  WHERE id = 'd0b22c8d-ac37-41b3-9c8b-45eb174f29c6';  -- Big Bay
UPDATE public.tables SET priority =  80, event_priority =  10, min_party_size = 1
  WHERE id = 'ea61faf9-ebfc-4964-bd60-ef907af36848';  -- Low 4a
UPDATE public.tables SET priority =  90, event_priority =  20, min_party_size = 1
  WHERE id = '8ff55f2a-86cb-4b2d-ae74-2d8cae44499b';  -- Low 4b
UPDATE public.tables SET priority = 100, event_priority =  30, min_party_size = 1,
...
UPDATE public.tables SET priority = 999, event_priority = 999, min_party_size = 1
...
ALTER TABLE public.tables
  ADD CONSTRAINT tables_priority_positive CHECK (priority >= 0 AND event_priority >= 0);
...
BEGIN
  ...
  IF NOT (
    (SELECT event_priority FROM public.tables WHERE id = '39350c06-d5ea-4cea-a742-9ea78ebc0557')
      > (SELECT event_priority FROM public.tables WHERE id = 'ea61faf9-ebfc-4964-bd60-ef907af36848')
  ) THEN
