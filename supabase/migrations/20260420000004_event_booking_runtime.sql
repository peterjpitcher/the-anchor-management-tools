-- v0.5 event booking runtime helpers
-- Adds atomic capacity snapshot + booking/waitlist creation functions.
...
AS $$
  ...
  booking_totals AS (
    SELECT
      ...
      SUM(
        CASE
          WHEN b.status = 'confirmed' THEN COALESCE(b.seats, 0)
          WHEN b.status = 'pending_payment' AND (b.hold_expires_at IS NULL OR b.hold_expires_at > NOW()) THEN COALESCE(b.seats, 0)
          ELSE 0
        END
      )::integer AS reserved_seats
    FROM public.bookings b
    JOIN filtered_events fe ON fe.id = b.event_id
    GROUP BY b.event_id
  ),
  waitlist_holds AS (
    SELECT
      wo.event_id,
      SUM(bh.seats_or_covers_held)::integer AS held_seats
    FROM public.booking_holds bh
    JOIN public.waitlist_offers wo ON wo.id = bh.waitlist_offer_id
    JOIN filtered_events fe ON fe.id = wo.event_id
    WHERE bh.status = 'active'
      AND bh.hold_type = 'waitlist_hold'
      AND bh.expires_at > NOW()
    GROUP BY wo.event_id
  )
  SELECT
    fe.id AS event_id,
    fe.capacity,
    COALESCE(bt.reserved_seats, 0) AS confirmed_seats,
    COALESCE(wh.held_seats, 0) AS held_seats,
    CASE
      WHEN fe.capacity IS NULL THEN NULL
      ELSE GREATEST(fe.capacity - COALESCE(bt.reserved_seats, 0) - COALESCE(wh.held_seats, 0), 0)
    END AS seats_remaining,
    CASE
      WHEN fe.capacity IS NULL THEN FALSE
      ELSE (fe.capacity - COALESCE(bt.reserved_seats, 0) - COALESCE(wh.held_seats, 0)) <= 0
    END AS is_full
  FROM filtered_events fe
  LEFT JOIN booking_totals bt ON bt.event_id = fe.id
  LEFT JOIN waitlist_holds wh ON wh.event_id = fe.id;
$$;
...
BEGIN
  ...
  IF v_event.capacity IS NULL OR v_event.capacity < 1 THEN
    RETURN jsonb_build_object(
      'state', 'blocked',
  ...
  IF v_capacity_snapshot.seats_remaining IS NULL OR v_capacity_snapshot.seats_remaining < p_seats THEN
    RETURN jsonb_build_object(
  ...
  RETURN jsonb_build_object(
    ...
    'seats_remaining', GREATEST(v_capacity_snapshot.seats_remaining - p_seats, 0)
  );
END;
$$;
...
BEGIN
  ...
  IF COALESCE(v_capacity_snapshot.seats_remaining, 0) >= p_requested_seats THEN
    RETURN jsonb_build_object(
      'state', 'not_full',
