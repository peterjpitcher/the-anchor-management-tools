-- Fix for casting errors in create_table_booking_transaction
-- The issue was that JSONB -> text extraction was not being explicitly cast to the ENUM types
...
BEGIN
  ...
  INSERT INTO table_bookings (
    ...
    (p_booking_data->>'booking_type')::table_booking_type, -- Fixed: Explicit cast to enum
    p_booking_data->>'special_requirements',
    CASE 
      WHEN p_booking_data->'dietary_requirements' IS NULL OR jsonb_typeof(p_booking_data->'dietary_requirements') = 'null' THEN NULL 
      ELSE (
        SELECT array_agg(x) 
        FROM jsonb_array_elements_text(p_booking_data->'dietary_requirements') t(x)
      )
    END,
    CASE 
      WHEN p_booking_data->'allergies' IS NULL OR jsonb_typeof(p_booking_data->'allergies') = 'null' THEN NULL 
      ELSE (
        SELECT array_agg(x) 
        FROM jsonb_array_elements_text(p_booking_data->'allergies') t(x)
      )
    END,
    p_booking_data->>'celebration_type',
    COALESCE((p_booking_data->>'duration_minutes')::INTEGER, 120),
    COALESCE(p_booking_data->>'source', 'phone'),
    COALESCE(p_booking_data->>'status', 'confirmed')::table_booking_status -- Fixed: Explicit cast to enum
  )
  ...
  IF p_payment_data IS NOT NULL THEN
    INSERT INTO table_booking_payments (
      ...
      (p_payment_data->>'status')::payment_status, -- Fixed: Explicit cast to enum
