...
| `opening_hours_week` *(Sept 2026)* | `heading`, `rows[{day, bar, exception?, kitchen \| lunch+dinner \| closed}]`, `footnote` | Seven-day table, two columns (Day 112px, details). Details cell carries labelled nowrap lines: BAR, then LUNCH + DINNER, or KITCHEN, or muted KITCHEN Closed, then any exception. |
| `opening_hours_dates` *(Sept 2026)* | `heading`, `rows[{date, hours \| closed, note?}]`, `footnote` | Dated exceptions for festive runs. Two columns (date, hours + note). A closed day renders as a full dark `#0c1d11` row with gold date and cream "Closed". |
