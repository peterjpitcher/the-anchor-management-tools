...
**Goal:** Create the database schema, RLS policies, RBAC permissions, storage bucket, and navigation entries shared by all three new modules (Mileage, Expenses, MGD).

**Architecture:** Single Supabase migration creates all tables with role-aware RLS via a shared `is_super_admin()` function. RBAC types updated in TypeScript. Navigation entries added to the Finance group. Storage bucket for expense receipts created with proper policies.

**Tech Stack:** Supabase (PostgreSQL), TypeScript, Next.js App Router
...
**Files:**
- Create: `supabase/migrations/20260405000000_mileage_expenses_mgd.sql`

- [ ] **Step 1: Create the migration file with is_super_admin function and mileage tables**
...
Expected: No errors. Migration should report creating 8 tables, the `is_super_admin` function, the `mgd_collection_sync_return` trigger function, the `set_updated_at` function, storage bucket, and seed data.
...
```bash
git add supabase/migrations/20260405000000_mileage_expenses_mgd.sql
git commit -m "feat: add database schema for mileage, expenses, and MGD modules"
```

---

### Task 2: Update RBAC TypeScript types

**Files:**
- Modify: `src/types/rbac.ts`

- [ ] **Step 1: Add the three new module names to ModuleName**

In `src/types/rbac.ts`, add `'mileage'`, `'expenses'`, and `'mgd'` to the `ModuleName` union type. Add them after `'payroll'` (the last existing entry):
...
git commit -m "feat: add mileage, expenses, mgd to RBAC ModuleName type"
```
...
**Files:**
- Modify: `src/components/features/shared/AppNavigation.tsx`

- [ ] **Step 1: Add icon imports**

At the top of `AppNavigation.tsx`, add imports for the icons (check which Heroicons are already imported and add any missing ones):

```typescript
import { MapIcon, MapPinIcon, CurrencyPoundIcon } from '@heroicons/react/24/outline'
```

Note: `BanknotesIcon` is already imported (used by Cashing Up). We'll reuse it for Expenses since it's the closest match, or use `ReceiptPercentIcon` if available. Check the existing imports and pick the best fit.

- [ ] **Step 2: Add navigation items to financeNavigation**

Insert the three new modules after the Receipts entry and before Short Links in the `financeNavigation` array:
...
Run: `npx tsc --noEmit`
Expected: Clean compilation. The new module names must match the `ModuleName` type from Task 2.

- [ ] **Step 4: Commit**

```bash
git add src/components/features/shared/AppNavigation.tsx
git commit -m "feat: add mileage, expenses, MGD to finance navigation"
```
...
**Files:**
- Create: `src/app/(authenticated)/mileage/page.tsx`
- Create: `src/app/(authenticated)/mileage/destinations/page.tsx`
- Create: `src/app/(authenticated)/expenses/page.tsx`
- Create: `src/app/(authenticated)/mgd/page.tsx`

- [ ] **Step 1: Create mileage main page**
...
- [ ] **Step 2: Create mileage destinations page**
...
Run: `npm run build`
Expected: Clean build. All four pages should be statically analysable.

- [ ] **Step 6: Commit**

```bash
git add src/app/\(authenticated\)/mileage/ src/app/\(authenticated\)/expenses/ src/app/\(authenticated\)/mgd/
git commit -m "feat: add placeholder pages for mileage, expenses, and MGD"
```

---

### Task 5: Update OJ-Projects default mileage rate

**Files:**
- Create: `supabase/migrations/20260405000001_update_oj_mileage_rate.sql`
...
- [ ] **Step 2: Update the UI default**

In `src/app/(authenticated)/oj-projects/clients/page.tsx`, find the default mileage rate value (search for `0.42`) and change it to `0.45`. This is in the form default and/or the new vendor settings initialisation.
...
```bash
git add supabase/migrations/20260405000001_update_oj_mileage_rate.sql src/app/\(authenticated\)/oj-projects/clients/page.tsx
git commit -m "feat: update OJ-Projects default mileage rate from £0.42 to £0.45"
```
...
- [ ] Navigation shows Mileage, Destinations, Expenses, MGD under Finance (for super_admin users)
- [ ] All four placeholder pages load with permission checks
- [ ] The Anchor row exists in mileage_destinations with is_home_base = TRUE
- [ ] 43 destinations seeded with distance cache populated
- [ ] RBAC permissions seeded and assigned to super_admin
- [ ] expense-receipts storage bucket created with super_admin-only RLS
