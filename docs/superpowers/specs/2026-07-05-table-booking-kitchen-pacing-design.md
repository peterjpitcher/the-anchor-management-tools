...
- Concurrency is already guarded by `pg_advisory_xact_lock` + the `booking_table_assignments` integrity trigger inside the RPC.
...
- The covers check is added **inside `create_table_booking_v05`**, after a table is allocated and before the insert, under the existing `pg_advisory_xact_lock` — so two simultaneous requests for the same slot cannot both pass (no oversell).
